...
Vercel 서버에서 공공데이터 원본 URL을 직접 호출하면 `403`으로 차단될 수 있습니다.
이 경우 공공데이터포털에서 CSV 파일을 직접 내려받아 프로젝트에 포함하세요.

1. 공공데이터포털에서 `행정안전부_CCTV정보` CSV를 다운로드합니다.
2. 파일명을 `cctv.csv`로 바꿉니다.
3. GitHub 웹 업로드 25MB 제한에 걸리면 `cctv.csv.gz`로 압축합니다.
4. 아래 위치 중 하나에 넣습니다.
...
5. GitHub에 다시 업로드하면 Vercel이 재배포합니다.
